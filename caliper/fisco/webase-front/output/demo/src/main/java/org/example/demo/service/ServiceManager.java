...
public class ServiceManager {
  ...
  /**
   * @notice: must use @Qualifier("NewEnergyService") with @Autowired to get this Bean
   */
  @Bean("NewEnergyService")
  public Map<String, NewEnergyService> initNewEnergyServiceManager() throws Exception {
    Map<String, NewEnergyService> serviceMap = new ConcurrentHashMap<>(this.hexPrivateKeyList.size());
    for (int i = 0; i < this.hexPrivateKeyList.size(); i++) {
    	...
    	NewEnergyService newEnergyService = new NewEnergyService();
    	newEnergyService.setAddress(this.config.getContract().getNewEnergyAddress());
    	newEnergyService.setClient(this.client);
    	...
    	newEnergyService.setTxProcessor(txProcessor);
    	serviceMap.put(userAddress, newEnergyService);
    }
    log.info("++++++++NewEnergyService map:{}", serviceMap);
    return serviceMap;
  }
}
